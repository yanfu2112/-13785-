...
if [ -f out/bin/capture_display.elf ] && [ -f out/bin/face_ai.elf ] && [ -f out/bin/access_control.elf ]; then
      mkdir -p ${k230_bin}/face_access/db
      cp -a ${k230_kmodel}/face_detect_640.kmodel ${k230_bin}/face_access
      cp -a ${k230_kmodel}/face_recognize.kmodel ${k230_bin}/face_access
      cp out/bin/capture_display.elf ${k230_bin}/face_access
      cp out/bin/face_ai.elf ${k230_bin}/face_access
      cp out/bin/access_control.elf ${k230_bin}/face_access
      cp face_access_system/start_face_access.sh ${k230_bin}/face_access
      cp face_access_system/stop_face_access.sh ${k230_bin}/face_access
      cp -a face_access_system/linux_service ${k230_bin}/face_access
      chmod +x ${k230_bin}/face_access/*.sh
      chmod +x ${k230_bin}/face_access/linux_service/*.sh
fi

# Keep the build directory so failed targets can be diagnosed and successful
# builds can be rebuilt incrementally.
